-- Migration: Add patient existence check to doctor_save_prescription_and_send
-- Problem: RPC fails with cryptic FK violation (23503) when patient_id doesn't exist
--          in hospital_patients (stale queue data / data inconsistency after table recreation)
-- Fix: Validate patient exists before insert; raise an identifiable error if not
...
DECLARE
    ...
    v_patient_name TEXT;
BEGIN
    -- Guard: validate patient exists before attempting FK-constrained insert.
    -- This converts a cryptic 23503 FK violation into a clear, catchable error.
