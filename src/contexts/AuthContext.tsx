    ...
    const checkSession = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          // FIX: If session error is due to stale refresh token, clear it and proceed cleanly
          if (error.message?.includes('Refresh Token') || error.message?.includes('refresh_token')) {
            console.warn('[AuthContext] Stale refresh token detected, clearing and proceeding with no session');
            try {
              localStorage.removeItem('supabase.auth.token');
            } catch (e) {
              // Ignore storage errors
            }
            // Don't throw - just proceed with no session
          } else {
            throw error;
          }
        }
        ...
        // FIX: If abort/timeout error, clear stale tokens to prevent future issues
        if (error instanceof Error && (error.name === 'AbortError' || error.message?.includes('abort'))) {
          console.warn('[AuthContext] Session check aborted, clearing stale tokens');
          try {
            localStorage.removeItem('supabase.auth.token');
          } catch (e) {
            // Ignore
