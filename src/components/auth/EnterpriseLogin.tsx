...
const EnterpriseLogin: React.FC<EnterpriseLoginProps> = ({ onSwitchToChooser }) => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // FIX: Clear stale Supabase auth tokens to prevent refresh_token_not_found errors
            // This is critical when switching between Supabase projects or after long inactivity
            try {
                const authKey = 'supabase.auth.token';
                const storedSession = localStorage.getItem(authKey);
                if (storedSession) {
                    console.log('[EnterpriseLogin] Clearing stale auth session before fresh login');
                    localStorage.removeItem(authKey);
                }
            } catch (e) {
                console.warn('[EnterpriseLogin] Could not clear stale auth token:', e);
            }
