...
import DoctorMobileBottomNav, { DoctorView } from './layout/DoctorMobileBottomNav';
...
const DoctorDashboardMain: React.FC = () => {
  ...
  const MessagesStatCard: React.FC<{ localCount: number }> = ({ localCount }) => {
    ...
    return (
      <div className="glass-panel skeuomorph-card relative overflow-hidden rounded-[2rem] p-6 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_16px_40px_rgba(99,102,241,0.12)]">
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-indigo-500/10 via-violet-400/6 to-transparent rounded-[2rem]" />
        <div className="relative flex flex-col h-full justify-between">
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-widest">Messages</h3>
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-50">
                <MessagesIcon className="w-5 h-5 text-indigo-600" />
              </div>
            </div>
            <span className="text-4xl font-bold text-slate-950 tracking-tight">{count}</span>
          </div>
          <p className="text-sm text-slate-500 mt-3">Awaiting response</p>
        </div>
      </div>
    );
  };

  const StatCard = ({ title, value, subtext, icon: Icon, colorClass, gradient, iconBg }: any) => (
    <div className="glass-panel skeuomorph-card relative overflow-hidden rounded-[2rem] p-6 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_16px_40px_rgba(15,23,42,0.08)]">
      <div className={`pointer-events-none absolute inset-0 bg-gradient-to-br ${gradient || 'from-emerald-500/10 via-teal-400/6 to-transparent'} rounded-[2rem]`} />
      <div className="relative flex flex-col h-full justify-between">
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-widest">{title}</h3>
            <div className={`flex h-9 w-9 items-center justify-center rounded-xl ${iconBg || 'bg-emerald-50'}`}>
              <Icon className={`w-5 h-5 ${colorClass}`} />
            </div>
          </div>
          <span className="text-4xl font-bold text-slate-950 tracking-tight">{value}</span>
        </div>
  ...
  const renderDashboard = () => (
    <div className="space-y-10 pb-12 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400 mb-3">Doctor Portal</p>
          <h1 className="text-4xl md:text-5xl font-bold text-slate-950 tracking-tight leading-tight">
            Welcome back,<br />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 xl:gap-8">
        <StatCard
          ...
          gradient="from-emerald-500/12 via-teal-400/6 to-transparent"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-semibold text-slate-950">Recent Patients</h2>
            <button
              onClick={() => setActiveView('monitoring')}
              className="text-sm font-semibold text-slate-500 hover:text-secondary-600 underline-offset-4 hover:underline transition-colors">
              View all
            </button>
          </div>
          ...
          <div className="rounded-2xl overflow-visible">
            {loading ? (
              ...
              <div className="glass-panel p-16 text-center rounded-[2rem]">
                ...
                <p className="text-slate-500 text-sm max-w-xs mx-auto">Share your referral code to start checking up on patients.</p>
              </div>
            ) : (
                ...
                {patients.slice(0, 4).map((patient, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className="glass-panel skeuomorph-card group relative overflow-hidden rounded-[2rem] p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(15,23,42,0.1)] cursor-pointer flex flex-col justify-between h-full"
                    >
                      {/* Top shimmer on hover */}
                      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-secondary-400/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3">
                          <div className="h-12 w-12 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-900 font-bold text-sm group-hover:scale-105 transition-transform duration-300">
                            {getInitials(patient.name, patient.email)}
                          </div>
                          <div>
                            <h4 className="font-semibold text-slate-950 text-base group-hover:text-secondary-600 transition-colors line-clamp-1">
                        ...
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-semibold ${config.bg} ${config.color}`}>
                          {config.label}
                        </span>
                      </div>
                      ...
                      <div className="space-y-2 mt-2">
                        <div className="bg-slate-50/80 rounded-xl p-3 flex items-center justify-between">
                          ...
                          <span className="text-xs font-semibold text-slate-800 truncate max-w-[120px]">{patient.condition || 'General'}</span>
                        </div>
                      </div>
  ...
  return (
    <NotificationProvider userId={user?.id || ''} activeView={activeView} userRole="doctor">
      <div className="page-shell min-h-screen font-sans text-slate-950 selection:bg-blue-500/20 selection:text-blue-900 relative flex flex-col">
        {/* Ambient orbs */}
        ...
        <div className="sticky top-0 z-50 w-full flex justify-center pointer-events-none">
          ...
          <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-[#f8fbff] via-[#f8fbff]/80 to-transparent" />

          {/* Main Header Pill */}
          <div className={`glass-panel pointer-events-auto relative mt-2 sm:mt-4 mx-2 sm:mx-4 w-full max-w-[1440px] transition-all duration-300 rounded-2xl sm:rounded-3xl ${activeView === 'messages' ? 'hidden md:flex' : ''}`}>
            <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 w-full">
              <div className="flex items-center justify-between h-16 sm:h-20">
                ...
                >
                  ...
                  <div className="flex flex-col justify-center">
                    <h2 className="text-lg md:text-xl font-bold leading-none tracking-tight">
                      <span className="text-slate-900">Bean</span>
                      <span className="text-secondary-500">Health</span>
                    </h2>
                    <p className="text-[9px] md:text-[10px] font-bold text-slate-400 tracking-[0.2em] mt-0.5">DOCTOR PORTAL</p>
                  </div>
                </div>

                {/* Center Tabs */}
                {/* Center Tabs - Desktop Only with Enhanced Mobile-style Animation */}
                <nav className="hidden md:flex absolute left-1/2 transform -translate-x-1/2 p-1 bg-white/60 backdrop-blur-xl border border-white/50 rounded-full ring-1 ring-slate-200/50 shadow-sm transition-all duration-500 ease-[cubic-bezier(0.19,1,0.22,1)]">
                  <button
                    onClick={() => setActiveView('dashboard')}
                    className={`group relative flex items-center justify-center h-10 rounded-[20px] overflow-hidden text-sm font-bold transition-all duration-500 ease-[cubic-bezier(0.19,1,0.22,1)] will-change-[width,transform,background-color] ${activeView === 'dashboard'
                      ? 'w-[130px] bg-slate-900 text-white scale-105'
                      : 'w-[100px] text-slate-500 hover:text-slate-800 hover:bg-slate-100/60 hover:scale-105 active:scale-95'
                      }`}
                  ...
                  <button
                    ...
                    className={`group relative flex items-center justify-center h-10 rounded-[20px] overflow-hidden text-sm font-bold transition-all duration-500 ease-[cubic-bezier(0.19,1,0.22,1)] will-change-[width,transform,background-color] ${(activeView === 'monitoring' || activeView === 'patient-detail')
                      ? 'w-[140px] bg-slate-900 text-white scale-105'
                      : 'w-[110px] text-slate-500 hover:text-slate-800 hover:bg-slate-100/60 hover:scale-105 active:scale-95'
                      }`}
                    style={{ WebkitTapHighlightColor: 'transparent' }}
                  >
                  ...
                  <button
                    ...
                    className={`group relative flex items-center justify-center h-10 rounded-[20px] overflow-hidden text-sm font-bold transition-all duration-500 ease-[cubic-bezier(0.19,1,0.22,1)] will-change-[width,transform,background-color] ${activeView === 'alerts'
                      ? 'w-[110px] bg-slate-900 text-white scale-105'
                      : 'w-[90px] text-slate-500 hover:text-slate-800 hover:bg-slate-100/60 hover:scale-105 active:scale-95'
                      }`}
                    style={{ WebkitTapHighlightColor: 'transparent' }}
                  >
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div className="h-8 w-px bg-slate-200/70 mx-1 sm:mx-2"></div>
                  ...
                  <div className="hidden md:block text-right pr-1">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 tracking-wide mt-1 uppercase">
                  ...
                  <button
                    ...
                    className="group flex items-center p-1 rounded-full bg-transparent hover:bg-slate-100/60 transition-all duration-500 ease-out"
                    aria-label="View Profile"
                  >
                    <div className="relative z-10">
                      <div className="h-9 w-9 md:h-10 md:w-10 bg-slate-900 rounded-full flex items-center justify-center text-white font-bold text-xs md:text-sm shadow-md ring-2 ring-white transition-colors duration-300 group-hover:ring-secondary-500">
                        {getInitials(profile?.name || user?.email || 'Dr', user?.email || '')}
                      </div>
                      {/* Online indicator */}
                      <span className="absolute bottom-0 right-0 block h-2.5 w-2.5 rounded-full ring-2 ring-white bg-secondary-500"></span>
                    </div>
                    ...
                    <div className="max-w-0 group-hover:max-w-[100px] overflow-hidden transition-all duration-500 ease-out opacity-0 group-hover:opacity-100">
                      <div className="flex items-center gap-2 pl-3 pr-2 whitespace-nowrap text-sm font-semibold text-slate-700">
        ...
        {/* Scroll fade mask */}
        <div className="pointer-events-none fixed top-2 sm:top-4 left-0 right-0 h-28 bg-gradient-to-b from-[#f8fbff] from-0% via-[#f8fbff]/70 via-40% to-transparent to-100% z-30"></div>
      </div>
