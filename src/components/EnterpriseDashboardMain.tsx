...
const EnterpriseDashboardMain: React.FC = () => {
    ...
    const fetchDoctors = async () => {
        ...
        try {
            ...
            // Sort doctors using config-driven strategy (falls back to alphabetical)
            const { data: profileData } = await (supabase
                .from('hospital_profiles' as any)
                .select('config')
                .eq('id', profile.id)
                .maybeSingle() as any);
            const sortOrder = profileData?.config?.doctor_sort_order || [];
            const sortedDoctors = sortDoctors((data || []) as any[], sortOrder);
