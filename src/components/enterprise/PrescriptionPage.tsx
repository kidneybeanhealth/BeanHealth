...
import { useTenant, getTenantDisplayName, getTenantPhone } from '../../contexts/TenantContext';
...
const PrescriptionPage: React.FC = () => {
    ...
    // Resolve specialist list: prefer tenant doctors, fall back to KKC defaults
    const SPECIALIST_OPTIONS = (() => {
        if (!tenant) return DEFAULT_SPECIALIST_OPTIONS;
        // If tenant config provides specialist list, use that
        return DEFAULT_SPECIALIST_OPTIONS; // Will be replaced when hospital_doctors table is wired
    })();
                                    ...
                                    {isFirstPage && (
                                        <div className="flex items-center justify-between border-b-2 border-black pb-1 mb-1">
                                            ...
                                            <div className="text-center flex-1">
                                                <h1 className="text-lg font-bold text-blue-900 leading-tight">{tenant ? getTenantDisplayName(tenant).toUpperCase() + ', ' + (tenant.city || '').toUpperCase() : 'CLINIC NAME'}</h1>
                                                {(!tenant || tenant.config?.enable_bilingual_prescription) && <h2 className="text-base font-bold text-blue-900 leading-tight">{tenant?.address || ''}</h2>}
                                            </div>
                                        </div>
                                    )}
                                        ...
                                        <div className="border-2 border-black mb-2 text-xs font-bold">
                                            <div className="flex border-b border-black min-h-[24px]">
                                                ...
                                                <div className="w-1/2 flex">
                                                    ...
                                                    <div className="flex-1 py-1 px-1.5 flex items-center">{patient?.age}{patient?.gender ? ` / ${patient?.gender}` : ''}</div>
                                                </div>
                                            </div>
                                            <div className="flex border-b border-black min-h-[24px]">
                                                <div className="w-1/2 flex border-r border-black">
                                    ...
                                    {isLastPage && (
                                        <div className="mt-auto">
                                            ...
                                            <div className="w-full border-2 border-black p-2 text-[12px] leading-[1.5] text-center font-bold mt-4">
                                                ...
                                                <p>Appt: {tenant ? getTenantPhone(tenant) : '0422-2494333, 73588 41555, 41666'} | Time: {tenant?.working_hours || '8am - 6pm'}</p>
                                                <p className="border-t border-gray-300 mt-1 pt-1">{tenant?.config?.footer_doctor_text || 'Dr. A. பிரபாகர் MD., DNB (Nephrology) | Dr. A. திவாகர் MS., M.ch (Urology)'}</p>
                                                <p>அவசர உதவிக்கு / Emergency: {tenant?.emergency_phone || '0422 - 2494333'} (24 மணி நேரமும் / 24 hrs Service)</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
