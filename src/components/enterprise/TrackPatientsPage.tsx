...
const TrackPatientsPage: React.FC<TrackPatientsPageProps> = ({ onBack, readOnly = false }) => {
    ...
    const [addPatientForm, setAddPatientForm] = useState({
        name: '', mrNumber: '', age: '', gender: '',
        fatherHusbandName: '', place: '', phone: '',
        reviewDate: '', doctorId: '', testsToReview: '',
    });
        ...
        try {
            const { data, error } = await supabase
                ...
                .select(`
                    ...
                    completed_at,
                    cancelled_at,
                    patient:hospital_patients(id, name, mr_number, phone, age, father_husband_name, place, token_number, beanhealth_id, app_access_enabled),
    ...
    const fetchDoctors = useCallback(async () => {
        ...
        try {
            const { data } = await (supabase as any)
                ...
                .eq('hospital_id', profile.id)
                .order('name');
            setDoctors((data || []) as { id: string; name: string; specialty: string }[]);
        } catch { /* noop */ }
    }, [profile?.id]);
    ...
    const handleToggleAppAccess = async (row: ReviewRow) => {
        if (!profile?.id || !row.patient) return;
        
        // Optimistic update locally
        const newAccessState = !row.patient.app_access_enabled;
        setReviews(prev => prev.map(r => r.patient_id === row.patient_id && r.patient ? 
            { ...r, patient: { ...r.patient, app_access_enabled: newAccessState } } : r
        ));

        toast.success(newAccessState ? 'App Access Enabled' : 'App Access Disabled');

        try {
            const { error } = await (supabase as any)
                .from('hospital_patients')
                .update({ app_access_enabled: newAccessState })
                .eq('id', row.patient_id)
                .eq('hospital_id', profile.id);

            if (error) throw error;
        } catch (err: any) {
            toast.error(err.message || 'Failed to update app access. Local state reverted.');
            // Revert on error
            setReviews(prev => prev.map(r => r.patient_id === row.patient_id && r.patient ? 
                { ...r, patient: { ...r.patient, app_access_enabled: !newAccessState } } : r
            ));
    ...
    const handleAddPatient = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!profile?.id) return;
        const name = addPatientForm.name.trim();
        if (!name) { toast.error('Patient name is required'); return; }
        setAddPatientSubmitting(true);
        try {
            const mrNumber = addPatientForm.mrNumber.trim() || null;
            if (mrNumber) {
                const { data: existing } = await (supabase as any)
                    ...
                    .maybeSingle();
                if (existing) {
                    toast.error('A patient with this MR number already exists');
            ...
            const { data: newPatient, error: patientError } = await (supabase as any)
                ...
                .insert({
                    ...
                    place: addPatientForm.place.trim() || null,
            ...
            setAddPatientForm({ name: '', mrNumber: '', age: '', gender: '', fatherHusbandName: '', place: '', phone: '', reviewDate: '', doctorId: '', testsToReview: '' });
            await fetchReviews();
        } catch (err: any) {
        ...
        <div className="min-h-screen bg-gray-100">
                ...
                <div className="flex flex-col gap-4 mb-6 sm:mb-8">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        <button
                            ...
                            className="px-4 py-2.5 bg-white text-gray-700 border border-gray-200 rounded-xl text-sm font-semibold hover:bg-gray-50"
                        >
                            Refresh
                        </button>
                        {!readOnly && (
                            <button
                                ...
                                className="flex items-center gap-2 px-4 py-2.5 bg-green-600 text-white rounded-xl text-sm font-semibold hover:bg-green-700 shadow-sm shadow-green-600/20 transition-colors"
                            >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            return (
                                                <div key={row.id} className={`grid ${bucketFilter === 'followup' ? 'grid-cols-[2.4rem_1.1fr_0.8fr_0.85fr_0.75fr_0.7fr_0.8fr_0.95fr_0.85fr_1.55fr]' : 'grid-cols-[2.4rem_1.1fr_0.8fr_0.85fr_0.75fr_0.7fr_0.8fr_0.95fr_1.55fr]'} gap-2 px-5 py-3.5 text-sm items-center hover:bg-gray-50 transition-colors`}>
                                                    ...
                                                    <div className="flex flex-col gap-1 pr-2">
                                                        <span className="text-gray-700 text-xs truncate font-mono block">{row.patient?.phone || '--'}</span>
                                                        <button 
                                                            onClick={(e) => { e.stopPropagation(); handleToggleAppAccess(row); }}
                                                            className={`flex items-center gap-1 w-fit rounded px-1.5 py-0.5 border text-[10px] font-bold transition-all ${
                                                                row.patient?.app_access_enabled 
                                                        ...
                                                        >
                                                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={row.patient?.app_access_enabled ? 2.5 : 2} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />
                                                            </svg>
                                                            {row.patient?.app_access_enabled ? 'App: ON' : 'App: OFF'}
                                                        </button>
                                                    </div>
                                                ...
                                                <div className="flex items-start justify-between gap-3 mb-3">
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="flex flex-wrap items-center gap-1.5 text-xs text-gray-600">
                                                            ...
                                                            <button 
                                                                onClick={(e) => { e.stopPropagation(); handleToggleAppAccess(row); }}
                                                                className={`flex items-center gap-1 shrink-0 rounded-full px-2 py-0.5 border text-[10px] font-bold transition-all ${
                                                                    row.patient?.app_access_enabled 
                                                            ...
                                                            >
                                                                ...
                                                                {row.patient?.app_access_enabled ? 'App Access: ON' : 'App Access OFF'}
                                                            </button>
                                                        </div>
                                                    </div>
            ...
            {showAddPatientModal && (
                <div className="fixed inset-0 z-[95] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] flex flex-col overflow-hidden">
                        <div className="px-6 py-4 border-b border-green-700/20 bg-gradient-to-r from-green-600 to-green-700 rounded-t-2xl flex items-center justify-between">
                            <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button onClick={() => setShowAddPatientModal(false)} className="text-white/70 hover:text-white p-1.5 rounded-lg hover:bg-white/10 transition-colors">
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                            </button>
                        </div>

                        <form onSubmit={handleAddPatient} className="flex-1 overflow-y-auto p-6 space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="sm:col-span-2">
                                    ...
                                    <input
                                        ...
                                        onChange={e => setAddPatientForm(f => ({ ...f, name: e.target.value }))}
                                        placeholder="Patient Name"
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none text-gray-900"
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">MR Number</label>
                                    <input
                                        type="text"
                                        value={addPatientForm.mrNumber}
                                        onChange={e => setAddPatientForm(f => ({ ...f, mrNumber: e.target.value }))}
                                        placeholder="MR-12345"
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none text-gray-900"
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Age</label>
                                    <input
                                        type="number"
                                        min={0}
                                        max={150}
                                        value={addPatientForm.age}
                                        onChange={e => setAddPatientForm(f => ({ ...f, age: e.target.value }))}
                                        placeholder="Years"
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none text-gray-900"
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Gender</label>
                                    <select
                                        value={addPatientForm.gender}
                                        onChange={e => setAddPatientForm(f => ({ ...f, gender: e.target.value }))}
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none text-gray-900"
                                    >
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setAddPatientForm(f => ({ ...f, phone: e.target.value }))}
                                        placeholder="Phone Number"
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none text-gray-900"
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Father / Husband Name</label>
                                    <input
                                        type="text"
                                        value={addPatientForm.fatherHusbandName}
                                        onChange={e => setAddPatientForm(f => ({ ...f, fatherHusbandName: e.target.value }))}
                                        placeholder="Father or Husband Name"
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none text-gray-900"
                                    />
                                </div>
                                <div className="sm:col-span-2">
                                    <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Place</label>
                                    <input
                                        type="text"
                                        value={addPatientForm.place}
                                        onChange={e => setAddPatientForm(f => ({ ...f, place: e.target.value }))}
                            ...
                            <div className="border-t border-gray-100 pt-4">
                                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Review Scheduling (optional)</p>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Review Date</label>
                                        <input
                                            type="date"
                                            value={addPatientForm.reviewDate}
                                            onChange={e => setAddPatientForm(f => ({ ...f, reviewDate: e.target.value }))}
                                            min={toISODateLocal(new Date())}
                                            className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none text-gray-900"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Consulting Doctor</label>
                                        <select
                                            value={addPatientForm.doctorId}
                                            onChange={e => setAddPatientForm(f => ({ ...f, doctorId: e.target.value }))}
                                            className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none text-gray-900"
                                        >
                                    ...
                                    <div className="sm:col-span-2">
                                        <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wide mb-1.5">Tests / Investigations to Review</label>
                                        <input
                                            type="text"
                                            value={addPatientForm.testsToReview}
                                            onChange={e => setAddPatientForm(f => ({ ...f, testsToReview: e.target.value }))}
                                            placeholder="e.g. CBC, RFT, Urine Protein"
                                            className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none text-gray-900"
                                        />
                                    </div>
                                </div>
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <button
                                    ...
                                    className="flex-1 py-2.5 bg-green-600 text-white rounded-xl font-semibold hover:bg-green-700 disabled:opacity-60 transition-colors shadow-sm"
                                >
