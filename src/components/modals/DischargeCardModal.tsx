...
import MobileDischargeCardInput from './MobileDischargeCardInput';
import {
...
interface DischargeCardModalProps {
  doctor: any;
  patient: any;
  onClose: () => void;
...
const DischargeCardModal: React.FC<DischargeCardModalProps> = ({
  doctor,
  patient,
  onClose,
  onSendToPharmacy,
  ...
  // Form States matching the Discharge Card PDF structure.
  // Note: `allergy` and `diagnosis` keys are kept for compatibility with the
  // existing modal logic (autocomplete, send-to-pharmacy persistence, etc.),
  // but the discharge card surfaces them as "Consultants" and "Final Diagnosis".
  ...
  useEffect(() => {
    ...
    if (existingData && !hasInitializedFromExisting.current) {
      ...
      try {
        ...
        const dateOfAdmission = extractField(notes, 'DateOfAdmission:');
        const dateOfDischarge = extractField(notes, 'DateOfDischarge:');
        const claim = extractField(notes, 'Claim:');
        const mlc = extractField(notes, 'MLC:');
        ...
        setFormData(prev => ({
          ...
          attenderName: attenderName || prev.attenderName,
          dateOfAdmission: dateOfAdmission ? dateOfAdmission.split('T')[0] : prev.dateOfAdmission,
          dateOfDischarge: dateOfDischarge ? dateOfDischarge.split('T')[0] : prev.dateOfDischarge,
  ...
  const confirmSendToPharmacy = () => {
    ...
    // diagnosis library. Diagnoses still print on the discharge card; to add a new
      ...
      <div className="bg-white w-full max-w-5xl rounded-3xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden animate-scale-in">
              ...
              return chunks.map((chunk, pageIndex) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* DISCHARGE CARD Title Banner - Only on Page 1 */}
                    {pageIndex === 0 && (
                      <div className="border-2 border-black mb-2 text-center py-1 font-bold text-sm tracking-widest bg-gray-50 print:bg-white">
                        DISCHARGE CARD
                      </div>
                    )}
                    ...
                    {pageIndex === 0 && (
                      <div className="border-2 border-black mb-2 text-xs font-bold" style={{ borderCollapse: 'collapse' }}>
                        {/* Row 1: NAME | AGE | MR NO — flex-[2]/flex-1/flex-1 so 50% and 75%
                            vertical dividers align with Row 2 (DOA/DOD/CLAIM/MLC, which has
                            dividers at 25/50/75%) and Row 3 (CONSULTANTS/ROOM NO at 75%). */}
                        ...
                        {/* Row 3: CONSULTANTS | ROOM NO — flex-[3]/flex-1 puts divider at 75%
                            which aligns with the 75% divider in Row 1 and Row 2. */}
                        <div className="flex border-b border-black min-h-[24px]">
                          <div className="flex-[3] flex border-r border-black min-w-0">
                            <div className="w-28 py-1 px-1.5 border-r border-black bg-gray-50 print:bg-white flex items-center text-[10px] leading-tight">ஆலோசகர்கள் / CONSULTANTS</div>
                            <input
                              className="flex-1 py-1 px-1.5 outline-none font-bold bg-transparent uppercase min-w-0"
                          ...
                          <div className="flex-1 flex min-w-0">
                            <div className="w-16 py-1 px-1.5 border-r border-black bg-gray-50 print:bg-white flex items-center text-[10px]">ROOM NO.</div>
                            <input
                              className="flex-1 py-1 px-1.5 outline-none font-bold bg-transparent uppercase min-w-0"
                              value={formData.roomNo}
                              onChange={e => setFormData({ ...formData, roomNo: e.target.value.toUpperCase() })}
                              readOnly={readOnly}
                            />
                          </div>
                        </div>
                        {/* Row 4: FINAL DIAGNOSIS — full width, label width matches column 1 of Row 1 / Row 3 */}
                        <div className="flex min-h-[48px]">
                          <div className="flex-1 flex relative">
                            <div className="w-28 py-1 px-1.5 border-r border-black bg-gray-50 print:bg-white flex items-center text-[10px] leading-tight">இறுதி வியாதிகள் / FINAL DIAGNOSIS</div>
                    ...
                    {isLastPage && (() => {
                      ...
                      return (
                        <div className="mt-auto">
                          ...
                          {/* Explanation acknowledgement (above the monitored block) */}
                          <div className="border-t border-black pt-2 mt-1 mb-2 text-sm font-bold">
                            <div className="flex flex-wrap items-baseline gap-1 uppercase">
                              <span>MEDICINE PRESCRIPTION DETAILS EXPLAINED TO PATIENT ATTENDER BY</span>
                              <input
                                className="flex-1 min-w-[120px] border-b border-gray-400 border-dotted outline-none bg-transparent px-1 uppercase"
                                value={formData.explainedBy}
                                onChange={e => !readOnly && setFormData({ ...formData, explainedBy: e.target.value.toUpperCase() })}
                                readOnly={readOnly}
                              />
                            </div>
                            <div className="flex flex-wrap items-baseline gap-2 mt-1 uppercase">
                              <span>1) NAME</span>
                              <input
                                className="flex-1 min-w-[120px] border-b border-gray-400 border-dotted outline-none bg-transparent px-1 uppercase"
                                value={formData.attenderName}
                                onChange={e => !readOnly && setFormData({ ...formData, attenderName: e.target.value.toUpperCase() })}
                                readOnly={readOnly}
                              />
                              <span>SIGN</span>
                              <span className="flex-1 min-w-[100px] border-b border-gray-400 border-dotted">&nbsp;</span>
                            </div>
                          </div>

                          {/* Specified / Monitored + Review block — two-column layout
                              Left: SALT / FLUID / DISCHARGE WT / REVIEW ON   Right: SPECIALISTS / TESTS */}
                          <div className={`border-t border-black pt-2 mt-1 ${scale.mb}`}>
                            ...
                            <div className={`flex gap-6 ${scale.textSize} font-bold`}>
                              {/* Left column: SALT, FLUID, DISCHARGE WT, REVIEW ON */}
                              ...
                              <div className={`flex-1 min-w-0 ${scale.spacing}`}>
                                <div className="flex gap-3 items-end">
                                  <div className="shrink-0 uppercase leading-tight flex flex-col">
                                    ...
                                    <span className="text-[10px] font-normal">மருத்துவர்</span>
                                  </div>
        ...
        <div className={`bg-white p-4 sm:p-6 border-t border-gray-100 flex flex-col sm:flex-row justify-end gap-3 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] ${isMobile && (showPrintView || showSendPreview) ? 'hidden' : 'flex'}`}>
          ...
          <div className="flex flex-1 sm:flex-none gap-3 order-1 sm:order-2">
            {/* In read-only viewers (Visit History at reception, pharmacy view),
                expose an explicit Download PDF action alongside Print. */}
            {readOnly && forcePrint && (
              <button
...
export default DischargeCardModal;
