...
const PrescriptionModal: React.FC<PrescriptionModalProps> = ({
  ...
  if (isMobile && !forceDesktop && !showPrintView && !showSendPreview) {
    return (
      <>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 overflow-y-auto animate-fade-in">
      {/* Floating Close Button for Mobile/Desktop top-right */}
      <button
        onClick={() => readOnly ? onClose() : setShowConfirmCloseModal(true)}
        className="fixed top-4 right-4 z-[100] bg-black/50 hover:bg-black/70 text-white rounded-full p-2.5 shadow-lg backdrop-blur-sm transition-all"
        aria-label="Close"
      >
      ...
      <div className="bg-white w-full max-w-5xl rounded-3xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden animate-scale-in relative">
                      ...
                      <div className="border-2 border-black mb-2 text-xs font-bold" style={{ borderCollapse: 'collapse' }}>
                        ...
                        <div className="flex border-b border-black min-h-[24px]">
                          ...
                          <div className="w-1/2 flex">
                            ...
                            <div className="flex-1 py-1 px-1.5 flex items-center">{(readOnly && existingData?.created_at) ? new Date(existingData.created_at).toLocaleDateString('en-GB') : new Date().toLocaleDateString('en-GB')}</div>
                          </div>
                        </div>
                        {/* Row 4 */}
                        <div className="flex border-b border-black min-h-[24px]">
        ...
        <div className={`bg-white p-4 sm:p-6 border-t border-gray-100 flex flex-col sm:flex-row justify-end gap-3 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] ${isMobile && (showPrintView || showSendPreview) ? 'hidden' : 'flex'}`}>
          <button onClick={() => readOnly ? onClose() : setShowConfirmCloseModal(true)} className="w-full sm:w-auto px-6 py-3 text-gray-600 font-bold hover:bg-gray-50 rounded-xl transition-colors order-2 sm:order-1">
            Cancel
          </button>
