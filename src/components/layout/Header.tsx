          ...
          <div className="w-full flex items-center justify-between px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
              ...
              >
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <p className="text-[8px] font-bold text-[#717171] dark:text-[#a0a0a0] tracking-[0.2em] mt-0.5 uppercase">{t.header.patientPortal}</p>
                </div>
              </div>
              ...
              >
                <h2 className="text-sm sm:text-lg md:text-xl font-bold text-gray-900 dark:text-white tracking-tight truncate">
                  {t.header.welcome}, <span className="text-secondary-500">{user.name}</span>
                </h2>
                <p className="text-[9px] sm:text-xs font-semibold text-gray-400 dark:text-gray-500 truncate tracking-wide uppercase opacity-90">
                  {user.role === 'patient' ? t.header.yourHealthDashboard : t.header.managePatients}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-full border border-gray-200 dark:border-white/10 bg-white/80 dark:bg-white/5 px-2.5 py-1.5 text-[11px] font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 transition-all duration-200 select-none"
                title={language === 'en' ? 'Switch to Tamil' : 'Switch to English'}
              >
                <span className={language === 'en' ? 'text-gray-900 dark:text-white' : 'text-gray-400'}>EN</span>
                <span className="text-gray-300 dark:text-gray-600 mx-0.5">|</span>
                <span className={language === 'ta' ? 'text-gray-900 dark:text-white' : 'text-gray-400'}>த</span>
              </button>
