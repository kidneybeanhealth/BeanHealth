...
| `phoneUtils.ts` | — | Indian mobile → E.164. **Mirrors `normalize_indian_mobile()` in `sql/20260814_phone_e164.sql`** — change both together. Client copy exists only for inline feedback at the point of typing; the DB is the source of truth |
...
| Table | Key Columns | Notes |
|-------|-------------|-------|
| `hospital_patients` | `hospital_id`, `name`, `age`, `token_number`, `linked_user_id`, `phone`, `phone_e164`, `mr_number` | Walk-in patients; `linked_user_id → users.id` for BeanHealth ID cross-linking. **`phone_e164` is GENERATED ALWAYS — never write it.** NULL = not reachable on WhatsApp |
...
- `sql/20260814_phone_e164.sql` — `normalize_indian_mobile()` + generated `hospital_patients.phone_e164`. Phase 1 of the WhatsApp reminder work
- `sql/whatsapp_readiness_audit.sql` — **read-only**, not a migration. Phone coverage / format spread / message volume / call-round baseline
...
| 2026-08-14 | **Phone numbers became a reachability fact instead of a free-text field.** Phase 1 of the WhatsApp review-reminder work — no sending, no scheduling; this only makes "can we reach this patient?" answerable. Four code paths write `hospital_patients.phone` (past-registration insert, walk-in insert, walk-in update, edit modal) and all four stored whatever reception typed, while the *lookup* path stripped to digits — so search tolerated mess that a send path cannot. Normalizing in the app would mean getting it right in four places plus every path added later, which is the drift that produced two Past Records cards and two copies of `getReviewFilterLabel`. Instead `phone_e164` is `GENERATED ALWAYS AS (normalize_indian_mobile(phone)) STORED`: unbypassable, backfilled implicitly by the ADD, and free for future write paths. `src/utils/phoneUtils.ts` mirrors the same rules client-side **for inline feedback only** — it deliberately does *not* gate the save, because refusing to register someone standing at the desk over a phone format is worse than the problem. Past Records grew a `No phone` / `Phone not usable` chip, which distinguishes `null` (unreachable) from `undefined` (migration not yet applied) — conflating them would paint a warning on every patient in the clinic. The service's optional-column degradation was also reworked to drop **one group at a time**: the previous single lifecycle flag would, on a site with lifecycle columns but no `phone_e164` (i.e. everyone until the migration runs), silently drop deceased/continuity data from Past Records — a regression that reads as data loss rather than a missing migration | `sql/20260814_phone_e164.sql`, `sql/whatsapp_readiness_audit.sql`, `src/utils/phoneUtils.ts`, `src/services/enterpriseReviewService.ts`, `src/components/enterprise/PastRecordsPatientCard.tsx`, `src/components/enterprise/ReceptionDashboard.tsx`, `CLAUDE.md` |
