...
interface LoginProps {
  ...
  selectedRole?: 'patient' | 'doctor' | 'hospital';
}

const Login: React.FC<LoginProps> = ({ onSwitchToChooser, selectedRole = 'patient' }) => {
  ...
  const isManual = selectedRole === 'doctor' || selectedRole === 'hospital';

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="text-center">
        <h2 className="text-2xl font-semibold mb-1 !text-gray-900 capitalize">
          {selectedRole} Login
        </h2>
        <p className="text-sm !text-gray-500">
          {isManual ? 'Sign in with your manual credentials' : 'Use your Google account to continue'}
        </p>
      </div>
        ...
        <form onSubmit={handleManualSignIn} className="space-y-4">
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full p-4 rounded-2xl border-2 border-gray-100 bg-gray-50 focus:border-secondary-500 focus:bg-white transition-all outline-none text-gray-900"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 rounded-full bg-secondary-500 hover:bg-secondary-600 text-white font-bold text-lg shadow-lg shadow-secondary-200 transition-all disabled:opacity-50 active:scale-95"
          >
