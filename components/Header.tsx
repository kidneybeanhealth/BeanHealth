...
const Header: React.FC<HeaderProps> = ({ user, onLogout, onMenuClick, onUpdateAvatar }) => {
  ...
  return (
    <header className="sticky top-0 z-40 h-14 sm:h-16 lg:h-20 bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl flex-shrink-0 border-b border-gray-200/60 dark:border-gray-700/60 flex items-center justify-between px-3 sm:px-4 lg:px-6 xl:px-8 shadow-sm">
        <div className="flex items-center space-x-2 sm:space-x-3 lg:space-x-4 flex-1 min-w-0">
             <button 
              onClick={onMenuClick} 
              className="md:hidden p-1.5 sm:p-2 rounded-lg sm:rounded-xl text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:scale-105 active:scale-95 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-rose-900"
              aria-label="Toggle menu"
            >
                <MenuIcon className="h-5 w-5 sm:h-6 sm:w-6" />
            </button>
            <div className="animate-fade-in min-w-0 flex-1">
                <h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-display font-bold text-gray-900 dark:text-gray-100 tracking-tight truncate">
                  Welcome back, <span className="text-rose-700 dark:text-rose-400">{user.name}</span>
                </h2>
                <p className="hidden sm:block text-xs lg:text-sm text-gray-600 dark:text-gray-400 mt-0.5 lg:mt-1 truncate">
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-2 lg:space-x-3 flex-shrink-0">
            ...
            <button 
              onClick={onLogout} 
              className="flex items-center justify-center p-1.5 sm:p-2 lg:p-2.5 rounded-lg sm:rounded-xl text-gray-600 dark:text-gray-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 hover:scale-105 active:scale-95 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-red-500"
              aria-label="Log out"
            >
