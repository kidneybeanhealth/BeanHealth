...
export default function WheelPicker({ values, value, onChange, disabled = false }) {
  ...
  const clamp = (v) => Math.max(0, Math.min(maxSt, v));
  ...
  // Wheel scroll (passive: false via ref)
  useEffect(() => {
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          background: disabled
            ? "linear-gradient(to bottom, #f7f7f6 20%, transparent)"
            : "linear-gradient(to bottom, #ffffff 20%, transparent)",
        }}
      />
      {/* Bottom fade */}
      <div
        ...
        style={{
          height: ITEM_H * CENTER,
          background: disabled
            ? "linear-gradient(to top, #f7f7f6 20%, transparent)"
            : "linear-gradient(to top, #ffffff 20%, transparent)",
        }}
      />
      {/* Center highlight band */}
      <div
        className="absolute inset-x-2 pointer-events-none rounded-xl transition-colors duration-300"
        style={{
          top: ITEM_H * CENTER,
          height: ITEM_H,
          background: disabled ? "hsl(120 3% 90%)" : "hsl(151 22% 31% / 0.07)",
          border: disabled ? "none" : "1px solid hsl(151 22% 31% / 0.18)",
        }}
      />
      ...
      <div style={{ transform: `translateY(${translateY}px)`, willChange: "transform" }}>
        {values.map((v, i) => {
          ...
          if (disabled) {
            cls = i === selIdx
              ? "text-muted-foreground font-medium text-xl"
              : dist === 1 ? "text-muted-foreground/40 text-lg"
              : "opacity-0 text-base";
          } else {
